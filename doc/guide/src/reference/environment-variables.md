...
| `KTSTR_SIDECAR_DIR` | Directory for per-test result sidecar JSON files. Used as-is when set, no kernel-key suffix is appended. Bare `cargo ktstr stats` reads this directory when set, but `cargo ktstr stats list` and `cargo ktstr stats compare` always enumerate `{CARGO_TARGET_DIR or "target"}/ktstr/` and ignore this variable -- runs written to a custom path are invisible to those subcommands. See [Runs](../running-tests/runs.md). | `{CARGO_TARGET_DIR or "target"}/ktstr/{kernel}-{git_short}/` |
