...
| `RUST_LOG` (host-side llama.cpp) | Controls the host-side tracing filter applied to llama.cpp internal log events. The first call to the inference engine's `global_backend()` initializer installs `tracing_subscriber::fmt::try_init()` (a no-op when an outer subscriber was already installed) and calls `llama_cpp_2::send_logs_to_tracing(LogOptions::default())` so every llama.cpp / GGML log line (model-load progress, GGUF parse chatter, KV-cache reservation notes, error reasons) flows into tracing as an event whose `Metadata::target()` is the literal string `"llama-cpp-2"`. The default `EnvFilter` derived from an unset `RUST_LOG` keeps only ERROR-level events, which is exactly the C-side rejection-reason text behind otherwise-opaque `InferenceError::ModelLoad` / `LlamaModelLoadError::NullResult` failures. To suppress all llama.cpp output, set `RUST_LOG=llama-cpp-2=off`; to widen verbosity, e.g. `RUST_LOG=llama-cpp-2=info` (note the literal hyphens — `EnvFilter` does prefix-matching on `meta.target()` without underscore normalization). Operators wanting a different sink (file, alternate format) can install their own subscriber FIRST — `try_init()` becomes a no-op and the operator's subscriber receives the events. | ERROR-level events emitted to stderr |
