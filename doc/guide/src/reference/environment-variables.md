...
Environment variables that control scx-ktstr behavior.
...
| `LLVM_PROFILE_FILE` | Standard LLVM profiling output path. scx-ktstr reads its parent as a fallback profraw directory. | None |

## Nextest protocol

| Variable | Description | Default |
|---|---|---|
| `NEXTEST` | Set by nextest. scx-ktstr intercepts `--list` and `--exact` args when present. | None |
