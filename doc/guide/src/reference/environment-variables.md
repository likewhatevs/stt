...
| `KTSTR_GHA_CACHE` | Set to `"1"` to enable remote kernel cache via GitHub Actions cache service. Requires `ACTIONS_CACHE_URL` (set by the GHA runner). The binary must be compiled with the `gha-cache` feature. Local cache is always authoritative; remote failures are non-fatal. | None (disabled) |
