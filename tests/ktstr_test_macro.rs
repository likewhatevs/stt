...
fn resolve_func_ip_known_symbol(ctx: &Ctx) -> Result<AssertResult> {
    let _ = ctx;
    let ip = scx_ktstr::resolve_func_ip("schedule");
    if let Some(addr) = ip
        && addr > 0
    {
...
fn find_registered_tests() {
    assert!(
        scx_ktstr::test_support::find_test("basic_topology_check").is_some(),
        "basic_topology_check should be registered in KTSTR_TESTS"
    );
    assert!(
        scx_ktstr::test_support::find_test("default_attrs_compile").is_some(),
...
fn entry_fields_match_attrs() {
    let entry = scx_ktstr::test_support::find_test("basic_topology_check").unwrap();
...
fn entry_default_fields() {
    let entry = scx_ktstr::test_support::find_test("default_attrs_compile").unwrap();
    assert_eq!(entry.topology.sockets, 1);
    assert_eq!(entry.topology.cores_per_socket, 2);
    assert_eq!(entry.topology.threads_per_core, 1);
    assert_eq!(entry.memory_mb, 2048);
...
fn entry_flags_match_attrs() {
    let entry = scx_ktstr::test_support::find_test("flags_attrs_compile").unwrap();
...
fn entry_topo_constraints_match_attrs() {
    let entry = scx_ktstr::test_support::find_test("topo_constraints_compile").unwrap();
...
const TOPO_SCHED: scx_ktstr::test_support::Scheduler =
    scx_ktstr::test_support::Scheduler::new("topo_test").topology(3, 6, 1);
...
fn entry_topo_inherit_full() {
    let entry = scx_ktstr::test_support::find_test("topo_inherit_full").unwrap();
...
fn entry_topo_inherit_partial() {
    let entry = scx_ktstr::test_support::find_test("topo_inherit_partial").unwrap();
    assert_eq!(entry.topology.sockets, 3);
    assert_eq!(entry.topology.cores_per_socket, 6);
...
fn entry_performance_mode_set() {
    let entry = scx_ktstr::test_support::find_test("performance_mode_compile").unwrap();
    assert!(
...
fn derive_scheduler_binary() {
    assert!(matches!(
        TEST_DERIVE.binary,
        scx_ktstr::test_support::SchedulerSpec::Name("test-binary")
    ));
}
...
fn entry_typed_flags_match() {
    let entry = scx_ktstr::test_support::find_test("typed_flags_compile").unwrap();
...
fn entry_mixed_flags_match() {
    let entry = scx_ktstr::test_support::find_test("mixed_flags_compile").unwrap();
    assert_eq!(entry.required_flags, &["alpha", "beta"]);
}
...
fn entry_derive_topo_inherit() {
    let entry = scx_ktstr::test_support::find_test("derive_topo_inherit").unwrap();
...
fn derive_empty_enum_binary() {
    assert!(matches!(
        EMPTY_SCHED.binary,
        scx_ktstr::test_support::SchedulerSpec::Name("empty-binary")
    ));
}
...
fn derive_minimal_defaults() {
    ...
    assert!(matches!(
        MINIMAL.binary,
        scx_ktstr::test_support::SchedulerSpec::None
    ));
