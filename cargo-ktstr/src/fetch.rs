...
pub struct AcquiredSource {
    ...
    pub source_type: ktstr::cache::SourceType,
...
pub(crate) fn arch_info() -> (&'static str, &'static str) {
...
fn print_download_size(response: &reqwest::blocking::Response, url: &str) {
    if let Some(len) = response.content_length() {
        ...
        eprintln!("cargo-ktstr: downloading {url} ({mb:.1} MB)");
    } else {
        eprintln!("cargo-ktstr: downloading {url}");
    }
}
...
fn download_stable_tarball(version: &str, dest_dir: &Path) -> Result<PathBuf, String> {
    ...
    eprintln!("cargo-ktstr: extracting tarball (xz)");
...
fn download_rc_tarball(version: &str, dest_dir: &Path) -> Result<PathBuf, String> {
    ...
    eprintln!("cargo-ktstr: extracting tarball (gzip)");
...
pub fn download_tarball(version: &str, dest_dir: &Path) -> Result<AcquiredSource, String> {
    ...
    Ok(AcquiredSource {
        ...
        git_hash: None,
        git_ref: None,
        source_type: ktstr::cache::SourceType::Tarball,
        is_temp: true,
        is_dirty: false,
    })
}
...
pub fn fetch_latest_stable_version() -> Result<String, String> {
    ...
    eprintln!("cargo-ktstr: fetching latest kernel version");
    ...
    eprintln!("cargo-ktstr: latest stable kernel: {version}");
...
pub fn git_clone(url: &str, git_ref: &str, dest_dir: &Path) -> Result<AcquiredSource, String> {
    let (arch, _) = arch_info();
    eprintln!("cargo-ktstr: cloning {url} (ref: {git_ref}, depth: 1)");
    ...
    Ok(AcquiredSource {
        ...
        source_type: ktstr::cache::SourceType::Git,
        is_temp: true,
        is_dirty: false,
    })
}
...
pub fn local_source(source_path: &Path) -> Result<AcquiredSource, String> {
    ...
    let (short_hash, is_dirty) = match gix::discover(&canonical) {
        ...
        Err(_) => {
            eprintln!(
                "cargo-ktstr: warning: {} is not a git repository, cannot detect dirty state",
                source_path.display()
            );
            (None, true)
        }
    ...
    if is_dirty {
        eprintln!("cargo-ktstr: warning: dirty tree detected, building without caching");
    }
    ...
    Ok(AcquiredSource {
        ...
        git_hash: short_hash,
        git_ref: None,
        source_type: ktstr::cache::SourceType::Local,
        is_temp: false,
        is_dirty,
    })
}
