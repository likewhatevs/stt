...
pub enum GroupBy {
    ...
    /// whose names share a strip-trailing-digit prefix (≥3 chars)
    /// cluster into one bucket whose internal join key is the
    /// prefix. The kworker workqueue parser runs first so kernel
    /// threads named `kworker/<cpu>:<id>-<wq>` collapse across CPUs
    /// into one `kworker/*-<wq>` bucket per workqueue. Singleton
    /// buckets revert to the literal thread name so a lone worker
    /// stays ungrouped. Display labels are generated by `grex` for
...
/// so two threads with the same prefix collapse into one bucket
/// regardless of which integer suffix they carry.
const PATTERN_KEY_DIGIT_PLACEHOLDER: &str = "{N}";

/// Per-CPU placeholder used by [`kworker_pattern_key`] in place of
/// the `<cpu>:<id>` segment so kworker threads from different
/// CPUs but the same workqueue collapse into one bucket.
const KWORKER_CPU_PLACEHOLDER: &str = "*";

/// Compute the pattern key for a thread-name (`comm`) under
/// [`GroupBy::Comm`] aggregation.
///
/// First runs the kworker workqueue parser
/// ([`kworker_pattern_key`]) so a `kworker/<cpu>:<id>-<wq>` name
/// joins its workqueue group regardless of which CPU produced it.
/// Names that the kworker parser declines fall through to the
/// general trailing-digit logic: a run of trailing ASCII digits is
/// stripped and replaced with [`PATTERN_KEY_DIGIT_PLACEHOLDER`]
/// (`{N}`). The replacement is accepted iff (a) at least one digit
/// was stripped and (b) the prefix that remains is at least
/// [`PATTERN_KEY_MIN_PREFIX`] chars long. When any gate rejects,
/// the input comm is returned unchanged so the thread groups by
/// literal name.
///
/// Examples:
/// - `tokio-runtime-worker-12` → `tokio-runtime-worker-{N}`.
/// - `worker_5` → `worker_{N}` (prefix length 7 ≥ 3).
/// - `cpu0` → `cpu{N}` (prefix `cpu` is exactly 3 chars).
/// - `worker7` → `worker{N}` (no separator required).
/// - `t1` → `t1` (prefix `t` length 1 < 3).
/// - `t-1` → `t-1` (prefix `t-` length 2 < 3).
/// - `kworker/u8:7` → `kworker/u8:{N}` (no `-<wq>` suffix; falls
///   through to digit-strip).
/// - `kworker/42:7-mm_percpu_wq` → `kworker/*-mm_percpu_wq`
///   (kworker parser collapses the per-CPU prefix).
fn pattern_key(comm: &str) -> String {
    if let Some(key) = kworker_pattern_key(comm) {
        return key;
    }
...
/// Recognize `kworker/<cpu>:<id>[H]{+|-}<workqueue>` (the
/// kernel-thread workqueue worker comm format from
/// `wq_worker_comm` in `kernel/workqueue.c`) and return a key that
/// collapses the per-CPU prefix. Bound workers carry
/// `kworker/<cpu>:<id>[H]` where `<cpu>` is the integer CPU number;
/// unbound workers carry `kworker/u<pool_id>:<id>`. When a worker
/// is associated with a pool_workqueue, `wq_worker_comm` decorates
/// the comm with the workqueue name: `+<wq>` while
/// `worker->current_work` is set (actively executing the work
/// item), `-<wq>` after the work completes (carried in
/// `worker->desc`, last-active). Both forms appear on a busy host;
/// a thread can transition from `+events` to `-events` between
/// snapshots without changing what it does, so both decorations
/// normalize to the same bucket key.
///
/// A workqueue name itself can contain hyphens (`btrfs-endio`,
/// `btrfs-delalloc`), so the workqueue suffix is taken as
/// everything after the FIRST `+` or `-` past the colon delimiter
/// — not the last. The two decorations are interchangeable for
/// grouping purposes; the discarded prefix never re-appears in the
/// emitted key.
///
/// Returns:
/// - `Some("kworker/*-<wq>")` when the format matches and the
///   workqueue suffix is non-empty. The emitted key always uses
///   `-` regardless of whether the input carried `+` or `-`, so
///   active and idle workers collapse to one bucket per workqueue.
/// - `None` otherwise — the caller falls through to the general
///   trailing-digit pattern. Bare `kworker/<cpu>:<id>` (no work
///   currently or previously associated, so no decoration) takes
///   the fall-through branch and joins under `kworker/<cpu>:{N}`.
///
/// Examples:
/// - `kworker/42:7-mm_percpu_wq` → `Some("kworker/*-mm_percpu_wq")`
///   (idle, last-active was mm_percpu_wq).
/// - `kworker/42:7+mm_percpu_wq` → `Some("kworker/*-mm_percpu_wq")`
///   (active; emitted key uses `-` so it joins the idle bucket).
/// - `kworker/u32:5-events_unbound` → `Some("kworker/*-events_unbound")`.
/// - `kworker/u32:5+events_unbound` → `Some("kworker/*-events_unbound")`.
/// - `kworker/0:1H-events_highpri` → `Some("kworker/*-events_highpri")`
///   (the high-priority `H` is absorbed into the discarded prefix).
/// - `kworker/3:0-btrfs-endio` → `Some("kworker/*-btrfs-endio")` (the
///   workqueue name's own `-` is preserved after the first split).
/// - `kworker/42:7` → `None` (no decoration; falls through).
/// - `kworker/0:1H` → `None` (no decoration; falls through).
/// - `kworker/R-events` → `None` (rescuer format has no colon; falls
///   through to literal because it has no trailing digits).
/// - `not-kworker` → `None` (no `kworker/` prefix).
fn kworker_pattern_key(comm: &str) -> Option<String> {
    let after_prefix = comm.strip_prefix("kworker/")?;
    let colon_idx = after_prefix.find(':')?;
    let after_colon = &after_prefix[colon_idx + 1..];
    let decoration_idx = after_colon.find(['-', '+'])?;
    let wq = &after_colon[decoration_idx + 1..];
    if wq.is_empty() {
        return None;
    }
    Some(format!("kworker/{KWORKER_CPU_PLACEHOLDER}-{wq}"))
}
...
mod tests {
    ...
    /// the digit run is preserved verbatim in the prefix (it is no
    /// longer required, but it is still part of the key when
    /// present). Multi-numeric strings only strip the INNERMOST
    /// trailing-digit run; embedded digits inside the prefix stay
    /// literal.
    #[test]
    fn pattern_key_strips_trailing_digits() {
        ...
        // Space-separated prefix is preserved verbatim.
    ...
    /// Prefix shorter than [`PATTERN_KEY_MIN_PREFIX`] (=3) rejects
    /// the strip — `t1` (prefix `t` is 1 char), `a0` (1 char), and
    /// `t-1` (prefix `t-` is 2 chars) all stay literal; `ab_5`
    /// (prefix `ab_` is 3 chars) and `abc0` (prefix `abc` is 3
    /// chars) pass. Pins both ends of the boundary including the
    /// separator-free single-char prefix case that gap-1 introduces.
    #[test]
    fn pattern_key_rejects_prefix_below_min_length() {
        assert_eq!(pattern_key("t1"), "t1");
        assert_eq!(pattern_key("a0"), "a0");
        assert_eq!(pattern_key("t-1"), "t-1");
        assert_eq!(pattern_key("ab_5"), "ab_{N}");
        assert_eq!(pattern_key("abc0"), "abc{N}");
    }

    /// `kworker/<cpu>:<id>` with no `-<wq>` suffix takes the
    /// trailing-digit fall-through. `kworker/u8:7` → trimmed prefix
    /// `kworker/u8:` is 11 chars ≥ 3, so the digit-strip applies.
    #[test]
    fn pattern_key_kworker_bare_uses_general_digit_strip() {
        assert_eq!(pattern_key("kworker/u8:7"), "kworker/u8:{N}");
        assert_eq!(pattern_key("kworker/42:7"), "kworker/42:{N}");
        // `kworker/0:1H` ends with non-digit `H`, so the
        // trailing-digit trim removes nothing → stays literal.
        // No `-<wq>` suffix, so the kworker workqueue parser also
        // declines.
        assert_eq!(pattern_key("kworker/0:1H"), "kworker/0:1H");
    }

    /// `kworker_pattern_key` collapses the per-CPU prefix and keeps
    /// the workqueue suffix verbatim. Pins the happy path for the
    /// three workqueue-bearing kworker shapes (bound, unbound, with
    /// `H` high-priority marker) and a workqueue name that itself
    /// contains a hyphen (`btrfs-endio`).
    #[test]
    fn kworker_pattern_key_collapses_per_cpu_prefix() {
        assert_eq!(
            kworker_pattern_key("kworker/42:7-mm_percpu_wq"),
            Some("kworker/*-mm_percpu_wq".to_string()),
        );
        assert_eq!(
            kworker_pattern_key("kworker/u32:5-events_unbound"),
            Some("kworker/*-events_unbound".to_string()),
        );
        // High-priority marker `H` sits in the discarded prefix.
        assert_eq!(
            kworker_pattern_key("kworker/0:1H-events_highpri"),
            Some("kworker/*-events_highpri".to_string()),
        );
        // Workqueue name's own `-` is preserved (split on the FIRST
        // `-` past the colon, not the last).
        assert_eq!(
            kworker_pattern_key("kworker/3:0-btrfs-endio"),
            Some("kworker/*-btrfs-endio".to_string()),
        );
    }

    /// Active workers carry `+<wq>` decoration (set by
    /// `wq_worker_comm` in `kernel/workqueue.c` while
    /// `worker->current_work` is set); idle workers carry `-<wq>`.
    /// Both must collapse to the same `kworker/*-<wq>` key so a
    /// thread that transitions from active to idle between two
    /// snapshots stays in the same bucket. The emitted key always
    /// uses `-` (canonical form) regardless of which decoration the
    /// input carried.
    #[test]
    fn kworker_pattern_key_active_decoration_normalizes_to_idle() {
        // `+<wq>`: actively executing.
        assert_eq!(
            kworker_pattern_key("kworker/42:7+mm_percpu_wq"),
            Some("kworker/*-mm_percpu_wq".to_string()),
        );
        ...
        // `+<wq>` with H high-priority marker.
        assert_eq!(
            kworker_pattern_key("kworker/0:1H+events_highpri"),
            Some("kworker/*-events_highpri".to_string()),
        );
        // `+<wq>` where the workqueue name itself contains a `-`:
        // the FIRST decoration char (the leading `+`) is consumed,
        // so the workqueue's own `-btrfs-endio` survives intact.
        assert_eq!(
            kworker_pattern_key("kworker/3:0+btrfs-endio"),
            Some("kworker/*-btrfs-endio".to_string()),
        );
        // The `+` and `-` forms produce identical keys: the active
        // and idle workers join the same bucket.
        assert_eq!(
            kworker_pattern_key("kworker/42:7+mm_percpu_wq"),
            kworker_pattern_key("kworker/43:8-mm_percpu_wq"),
        );
    }

    /// `kworker_pattern_key` declines names that don't carry the
    /// `kworker/<cpu>:<id>{+|-}<wq>` shape: missing `kworker/`
    /// prefix, missing `:` (rescuer format `kworker/R-<wq>`),
    /// missing decoration (bare kworker), or empty suffix after a
    /// trailing decoration char. None means the caller falls
    /// through to the general trailing-digit logic.
    #[test]
    fn kworker_pattern_key_declines_other_shapes() {
        // Not a kworker thread.
        assert_eq!(kworker_pattern_key("worker-7"), None);
        assert_eq!(kworker_pattern_key("not-kworker"), None);
        // Rescuer format has no colon.
        assert_eq!(kworker_pattern_key("kworker/R-events"), None);
        // Bare kworker (no work currently or previously associated).
        assert_eq!(kworker_pattern_key("kworker/42:7"), None);
        assert_eq!(kworker_pattern_key("kworker/0:1H"), None);
        // Trailing decoration with empty workqueue suffix.
        assert_eq!(kworker_pattern_key("kworker/42:7-"), None);
        assert_eq!(kworker_pattern_key("kworker/42:7+"), None);
        // Empty input.
        assert_eq!(kworker_pattern_key(""), None);
    }

    /// `pattern_key` integration: a kworker workqueue thread routes
    /// through the kworker parser BEFORE the trailing-digit logic.
    /// `kworker/42:7-mm_percpu_wq` ends in non-digit `q` so the
    /// general logic alone would leave it literal — without the
    /// kworker parser, per-CPU kworkers with the same workqueue
    /// would never group.
    #[test]
    fn pattern_key_routes_kworker_workqueue_through_parser() {
        assert_eq!(
            pattern_key("kworker/42:7-mm_percpu_wq"),
            "kworker/*-mm_percpu_wq",
        );
        assert_eq!(
            pattern_key("kworker/43:8-mm_percpu_wq"),
            "kworker/*-mm_percpu_wq",
        );
        assert_eq!(
            pattern_key("kworker/u32:5-events_unbound"),
            "kworker/*-events_unbound",
        );
    }
    ...
    /// kworker workqueue grouping: `kworker/42:7-mm_percpu_wq`
    /// across distinct CPUs collapses into one
    /// `kworker/*-mm_percpu_wq` bucket. Different workqueue
    /// suffixes do NOT merge — `mm_percpu_wq` and `events_highpri`
    /// each get their own bucket. Pin both axes (per-CPU
    /// collapse, per-workqueue separation).
    #[test]
    fn build_groups_comm_kworker_collapses_per_cpu_per_workqueue() {
        let snap = snap_with(vec![
            ...
            make_thread("kworker", "kworker/0:1H-events_highpri"),
            make_thread("kworker", "kworker/1:2H-events_highpri"),
        ]);
        let groups = build_groups(&snap, GroupBy::Comm, &[], None);
        assert_eq!(groups["kworker/*-mm_percpu_wq"].thread_count, 3);
        assert_eq!(groups["kworker/*-events_highpri"].thread_count, 2);
        assert_eq!(groups.len(), 2);
    }

    /// Active (`+<wq>`) and idle (`-<wq>`) decorations collapse
    /// to the same `kworker/*-<wq>` bucket. A thread that was
    /// actively executing in the baseline snapshot may be idle in
    /// the candidate (or vice versa) without crossing buckets —
    /// the per-snapshot decoration choice is incidental, the
    /// workqueue identity is what determines the group.
    #[test]
    fn build_groups_comm_kworker_active_and_idle_share_bucket() {
        let snap = snap_with(vec![
            // Mix of `+` (active) and `-` (idle) on the same workqueue.
            make_thread("kworker", "kworker/42:7+mm_percpu_wq"),
            make_thread("kworker", "kworker/43:8-mm_percpu_wq"),
            make_thread("kworker", "kworker/44:9+mm_percpu_wq"),
            // Different workqueue, also mixed decoration.
            make_thread("kworker", "kworker/0:1H-events_highpri"),
            make_thread("kworker", "kworker/1:2H+events_highpri"),
        ]);
        let groups = build_groups(&snap, GroupBy::Comm, &[], None);
        // All three mm_percpu_wq workers (2 active + 1 idle) join
        // the same bucket.
        assert_eq!(groups["kworker/*-mm_percpu_wq"].thread_count, 3);
        // Both events_highpri workers (1 active + 1 idle) join.
        assert_eq!(groups["kworker/*-events_highpri"].thread_count, 2);
        assert_eq!(groups.len(), 2);
    }

    /// Bare kworker (no `-<wq>` suffix) takes the trailing-digit
    /// fall-through path: `kworker/42:7` and `kworker/42:8` group
    /// under `kworker/42:{N}` (per the existing digit-strip rule),
    /// but `kworker/42:*` and `kworker/43:*` stay in distinct
    /// buckets — the bare-kworker per-CPU collapse is gap-3 and
    /// remains deferred. Pin both: bare-kworker uses general logic;
    /// distinct CPUs get distinct buckets (no over-merge).
    #[test]
    fn build_groups_comm_kworker_bare_uses_general_logic() {
        let snap = snap_with(vec![
            make_thread("kworker", "kworker/42:7"),
            make_thread("kworker", "kworker/42:8"),
            make_thread("kworker", "kworker/43:1"),
            make_thread("kworker", "kworker/43:2"),
        ]);
        let groups = build_groups(&snap, GroupBy::Comm, &[], None);
        assert_eq!(groups["kworker/42:{N}"].thread_count, 2);
        assert_eq!(groups["kworker/43:{N}"].thread_count, 2);
        assert_eq!(groups.len(), 2);
    }

    /// Mixed kworker fleet: workqueue-bearing kworkers route through
    /// the kworker parser (collapse across CPUs), bare kworkers route
    /// through the digit-strip (distinct buckets per CPU). Together
    /// they coexist without cross-contamination.
    #[test]
    fn build_groups_comm_kworker_mixed_fleet() {
        let snap = snap_with(vec![
            // Workqueue-bearing → kworker parser, collapse per-CPU.
            make_thread("kworker", "kworker/42:7-mm_percpu_wq"),
            make_thread("kworker", "kworker/43:8-mm_percpu_wq"),
            // Bare → digit-strip, per-CPU buckets remain.
            make_thread("kworker", "kworker/42:1"),
            make_thread("kworker", "kworker/42:2"),
        ]);
        let groups = build_groups(&snap, GroupBy::Comm, &[], None);
        assert_eq!(groups["kworker/*-mm_percpu_wq"].thread_count, 2);
        assert_eq!(groups["kworker/42:{N}"].thread_count, 2);
        assert_eq!(groups.len(), 2);
    }
