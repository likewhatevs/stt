...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "specifies flags but scheduler")]
    fn validate_entry_flags_no_decls_panics() {
        static SCHED_NO_FLAGS: Scheduler = Scheduler::new("noflags");
        let entry = KtstrTestEntry {
            name: "bad_no_decls",
            scheduler: &SCHED_NO_FLAGS,
            required_flags: &["any_flag"],
            ..KtstrTestEntry::DEFAULT
        };
        validate_entry_flags(&entry);
    }
    ...
    #[test]
    #[should_panic(expected = "unknown excluded_flag")]
    fn validate_entry_flags_unknown_excluded() {
        static SCHED: Scheduler = Scheduler::new("sched").flags(FLAGS_AB);
        let entry = KtstrTestEntry {
            name: "bad_excluded",
            scheduler: &SCHED,
            excluded_flags: &["nonexistent"],
            ..KtstrTestEntry::DEFAULT
        };
        validate_entry_flags(&entry);
    }
