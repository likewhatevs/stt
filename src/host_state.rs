...
/// `None` on syscall failure (EPERM, ESRCH).
///
/// # CPU-count cap
///
...
/// 1024-CPU cap is ONLY visible when the syscall path wins AND
/// the host exceeds 1024 CPUs. Tracked as a follow-up: rewrite
/// this helper to allocate a dynamically-sized cpu_set with
/// `CPU_ALLOC` / `CPU_ALLOC_SIZE` + `CPU_COUNT_S` so the
/// syscall path covers everything the kernel allocates. The
/// fallback keeps the 1024+-CPU case correct today; the TODO
/// narrows the path we use for the common case.
pub fn read_affinity(tid: i32) -> Option<Vec<u32>> {
    ...
    let cpus: Vec<u32> = (0..libc::CPU_SETSIZE as usize)
