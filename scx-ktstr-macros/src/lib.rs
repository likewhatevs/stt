...
///
/// The annotated function must have signature `fn(&scx_ktstr::scenario::Ctx) ->
/// anyhow::Result<scx_ktstr::assert::AssertResult>`. The macro:
///
...
pub fn ktstr_test(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let scheduler_tokens = match &scheduler {
        ...
        None => {
            quote! { &::scx_ktstr::test_support::Scheduler::EEVDF }
        }
    };
    ...
    let topology_tokens = quote! {
        ::scx_ktstr::test_support::Topology {
    ...
    let test_body = if expect_err {
        quote! {
            let result = ::scx_ktstr::test_support::run_ktstr_test(&#entry_name);
            assert!(
                result.is_err(),
        ...
        quote! {
            let _result = ::scx_ktstr::test_support::run_ktstr_test(&#entry_name).unwrap();
        }
    };
    ...
    let expanded = quote! {
        ...
        static #entry_name: ::scx_ktstr::test_support::KtstrTestEntry = ::scx_ktstr::test_support::KtstrTestEntry {
            name: #name_str,
            func: #inner_name,
            topology: #topology_tokens,
            constraints: ::scx_ktstr::test_support::TopologyConstraints {
            ...
            replicas: #replicas,
            assert: ::scx_ktstr::assert::Assert {
...
fn derive_scheduler_inner(input: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    for (i, fi) in flag_infos.iter().enumerate() {
        ...
        decl_statics.push(quote! {
            static #decl_ident: ::scx_ktstr::scenario::flags::FlagDecl = ::scx_ktstr::scenario::flags::FlagDecl {
                name: #name_str,
                args: &[#(#args),*],
    ...
    let mut builder_chain = quote! {
        ::scx_ktstr::test_support::Scheduler::new(#sched_name_str)
    };

    if let Some(ref binary) = sched_binary {
        builder_chain = quote! {
            #builder_chain.binary(::scx_ktstr::test_support::SchedulerSpec::Name(#binary))
        };
    }
    ...
    let expanded = quote! {
        ...
        static #flags_array_ident: &[&::scx_ktstr::scenario::flags::FlagDecl] = &[#(#decl_refs),*];

        const #const_name: ::scx_ktstr::test_support::Scheduler = #builder_chain;
